...
class OGrafGraphic extends HTMLElement {
  async load(_params) {
    return void 0;
  }
...
const styles = ".fiindo-produkt-video {\n  position: absolute;\n  inset: 0;\n  pointer-events: none;\n  font-family: 'Inter', system-ui, -apple-system, sans-serif;\n}\n\n.fiindo-produkt-video__logo {\n  position: absolute;\n  left: 60px;\n  bottom: 100px;\n  width: 150px;\n  height: 96px;\n  display: flex;\n  align-items: center;\n  justify-content: flex-start;\n  color: #ffffff;\n  font-weight: 900;\n  font-size: 34px;\n  letter-spacing: -0.5px;\n  transform-origin: left center;\n  opacity: 0;\n  filter:\n    drop-shadow(0 4px 14px rgba(0, 0, 0, 0.55))\n    drop-shadow(0 0 22px rgba(47, 185, 209, 0.55))\n    drop-shadow(0 0 48px rgba(47, 185, 209, 0.25));\n}\n\n.fiindo-produkt-video__separator {\n  position: absolute;\n  left: 240px;\n  bottom: calc(100px + 17.28px);\n  width: 3px;\n  height: 61.44px;\n  background: linear-gradient(\n    to bottom,\n    transparent 0%,\n    #2fb9d1 18%,\n    #2fb9d1 82%,\n    transparent 100%\n  );\n  box-shadow: 0 0 14px #2fb9d1, 0 0 28px rgba(47, 185, 209, 0.55);\n  border-radius: 2px;\n  opacity: 0;\n}\n\n.fiindo-produkt-video__bar {\n  position: absolute;\n  left: 267px;\n  right: 0;\n  bottom: 100px;\n  height: 96px;\n  padding-left: 28px;\n  padding-right: 80px;\n  display: flex;\n  flex-direction: column;\n  justify-content: center;\n  gap: 4px;\n  background: linear-gradient(\n    95deg,\n    rgba(255, 255, 255, 1) 0%,\n    rgba(255, 255, 255, 0.97) 35%,\n    rgba(232, 244, 248, 0.92) 60%,\n    rgba(232, 244, 248, 0.55) 80%,\n    rgba(232, 244, 248, 0) 100%\n  );\n  box-shadow:\n    inset 0 1px 0 rgba(255, 255, 255, 0.95),\n    inset 0 -1px 0 rgba(24, 49, 63, 0.08);\n  filter:\n    drop-shadow(0 14px 28px rgba(0, 0, 0, 0.42))\n    drop-shadow(0 0 48px rgba(47, 185, 209, 0.22));\n  clip-path: inset(0 100% 0 0);\n}\n\n.fiindo-produkt-video__title {\n  color: #18313f;\n  font-weight: 900;\n  font-size: 32px;\n  line-height: 1.05;\n  letter-spacing: -0.5px;\n  white-space: nowrap;\n  overflow: hidden;\n  opacity: 0;\n}\n\n.fiindo-produkt-video__description {\n  color: #5a6e7a;\n  font-weight: 500;\n  font-size: 19px;\n  line-height: 1.35;\n  max-width: 1100px;\n  opacity: 0;\n}\n";
const TAG_NAME = "fiindo-produkt-video";
const ENTRY_DURATION = 0.5;
const EXIT_DURATION = 0.3;
const MANIFEST_DEFAULT_ENTRIES = [
  { name: "Dashboard", role: "Entdecke fiindo – die App, die alles fürs Investieren bündelt.", duration: 1e4 },
  { name: "News", role: "Tagesaktuelle Märkte, prägnant zusammengefasst.", duration: 9e3 },
  { name: "Themen", role: "Investiere in deine Lieblingsthemen – von KI bis Klima.", duration: 4e3 },
  { name: "Top-Gewinner & -Verlierer", role: "Markt-Movers auf einen Blick.", duration: 1e4 },
  { name: "Watchlisten", role: "Behalte deine Werte im Auge – jederzeit, einen Tap entfernt.", duration: 1e4 },
  { name: "Watchlisten erstellen", role: "Lass die KI gemeinsam mit dir Watchlisten und Portfolios bauen.", duration: 1e4 },
  { name: "Watchlist mit KI analysieren", role: "KI-Briefing für deine Watchlist – bleib up to date.", duration: 1e4 },
  { name: "Brainstorming", role: "Neue Idee? Sag der KI deine These – sie findet passende Aktien.", duration: 1e4 },
  { name: "Aktienanalyse in einem Klick", role: "Eine kurze Übersicht zu jeder Aktie – ein Tap reicht.", duration: 1e4 },
  { name: "Ähnliche Aktien", role: "In Sekunden vergleichbare Aktien finden.", duration: 1e4 },
  { name: "Was macht eigentlich …?", role: "Was macht eigentlich Norwegian Cruise Line Holdings? Erfahre es mit einem Klick.", duration: 1e4 },
  { name: "Profi-Modus", role: "Wechsle in den Profi-Modus und analysiere noch tiefer.", duration: 1e4 },
  { name: "Earnings Flow", role: "Wie ist eigentlich der Umsatz von Microsoft aufgebaut?", duration: 4e3 },
  { name: "Kennzahlenanalyse", role: "Vergleiche Kennzahlen wie ein Profi.", duration: 1e4 },
  { name: "KI-Begleiter", role: "Die fiindo-KI begleitet dich überall – jede Frage sofort beantwortet.", duration: 1e4 },
  { name: "Analystenmeinungen", role: "Was denken die Profis über meine Aktie? Finde es einfach heraus.", duration: 1e4 },
  { name: "Fair Value", role: "Was ist eigentlich der faire Wert von … Nvidia?", duration: 1e4 },
  { name: "Chat", role: "Chatte direkt mit der KI und bekomme einfache Antworten auf deine Fragen.", duration: 1e4 },
  { name: "KI-Chat pro Aktie", role: "Direkte Frage zu einer Aktie? Nutze den integrierten KI-Chat zu jeder Aktie.", duration: 1e4 },
  { name: "fiindo Community", role: "Werde Teil der zukünftigen fiindo Community – jederzeit Austausch über Aktien und ETFs.", duration: 1e4 },
  { name: "Wissens-Quiz", role: "Lerne spielerisch und teste dein Wissen im fiindo Wissens-Quiz.", duration: 1e4 },
  { name: "Tiefenanalyse mit Stock Reports", role: "Tiefenanalysen mit den fiindo-Stock-Reports – 23 Kapitel pures Wissen, immer up to date.", duration: 1e4 },
  { name: "Earnings-Kalender", role: "Wer hatte gerade neue Quartalszahlen? Wie waren sie? Was steht als Nächstes an? Alles im Earnings-Kalender.", duration: 1e4 },
  { name: "Stock-Screener", role: "Frag in natürlicher Sprache nach Aktien – der fiindo Stock-Screener findet sie.", duration: 1e4 }
];
const DEFAULT_DATA = { entries: MANIFEST_DEFAULT_ENTRIES };
function toData(input) {
  if (input && typeof input === "object") {
    const src = input;
    if (Array.isArray(src.entries)) {
      const filtered = src.entries.filter(isEntry);
      if (filtered.length > 0) {
        return { entries: filtered };
      }
    }
  }
  return { entries: [...MANIFEST_DEFAULT_ENTRIES] };
}
function isEntry(value) {
  if (!value || typeof value !== "object") return false;
  const v = value;
  return typeof v.name === "string" && typeof v.role === "string" && typeof v.duration === "number";
}
class FiindoProduktVideo extends OGrafGraphic {
  data = { ...DEFAULT_DATA };
  dom = null;
  timeline = null;
  // -1 = before step 0, so first playAction({delta: 1}) lands on step 0.
  currentStep = -1;
  async load(params) {
    this.data = toData(params.data);
    this.buildDom();
    this.rebuildTimeline();
    if (this.data.entries.length > 0) {
      await this.playAction({ goto: 0, skipAnimation: true });
    }
    return void 0;
  }
  async playAction(params) {
    ...
    if (typeof params.goto === "number") {
      this.currentStep = clampStep(params.goto, total);
    } else {
      const delta = typeof params.delta === "number" ? params.delta : 1;
      this.currentStep = clampStep(this.currentStep + delta, total);
    }
    const tl = this.timeline;
    if (!tl) return { statusCode: 200, currentStep: this.currentStep };
    const label = stepLabel(this.currentStep);
  ...
  async stopAction(params) {
    if (!this.timeline || !this.dom) return void 0;
    if (params.skipAnimation) {
      this.resetTargets();
    } else {
      gsapWithCSS.to([this.dom.bar, this.dom.logo, this.dom.separator, this.dom.title, this.dom.description], {
        opacity: 0,
  ...
  async goToTime(params) {
    if (!this.timeline) return void 0;
    const seconds = params.timestamp / 1e3;
    this.timeline.pause().seek(seconds, false);
    return void 0;
  }
  async dispose() {
    if (this.timeline) {
  ...
  buildDom() {
    ...
    root.className = TAG_NAME;
    const logo = document.createElement("div");
    logo.className = `${TAG_NAME}__logo`;
    logo.textContent = "fiindo";
    const separator = document.createElement("div");
    separator.className = `${TAG_NAME}__separator`;
    const bar = document.createElement("div");
    bar.className = `${TAG_NAME}__bar`;
    const title = document.createElement("span");
    title.className = `${TAG_NAME}__title`;
    const description = document.createElement("span");
    description.className = `${TAG_NAME}__description`;
    ...
    this.dom = { root, logo, separator, bar, title, description };
  }
  rebuildTimeline() {
    if (!this.dom) return;
    if (this.timeline) {
    ...
    const { logo, separator, bar, title, description } = this.dom;
    this.data.entries.forEach((entry, index) => {
      const label = stepLabel(index);
      tl.addLabel(label);
      tl.call(() => {
        title.textContent = entry.name;
        description.textContent = entry.role;
      }, void 0, label);
      if (index === 0) {
        tl.set([logo, separator, title, description], { opacity: 1 }, label);
        tl.set(logo, { x: 0, scale: 1 }, label);
        tl.set(bar, { clipPath: "inset(0 0% 0 0)" }, label);
      } else {
        tl.set([logo, separator, title, description], { opacity: 0 }, label);
        tl.set(logo, { x: -12, scale: 0.88 }, label);
        tl.set(bar, { clipPath: "inset(0 100% 0 0)" }, label);
        tl.to(logo, { opacity: 1, x: 0, scale: 1, duration: ENTRY_DURATION, ease: "power2.out" }, label);
        tl.to(separator, { opacity: 1, duration: ENTRY_DURATION * 0.7, ease: "power1.out" }, `${label}+=${ENTRY_DURATION * 0.25}`);
        tl.to(bar, { clipPath: "inset(0 0% 0 0)", duration: ENTRY_DURATION * 0.9, ease: "power3.out" }, `${label}+=${ENTRY_DURATION * 0.3}`);
        tl.to([title, description], { opacity: 1, duration: ENTRY_DURATION * 0.6, ease: "power1.out", stagger: 0.05 }, `${label}+=${ENTRY_DURATION * 0.55}`);
      }
      const holdSec = Math.max(0, entry.duration / 1e3);
      tl.to({}, { duration: holdSec });
      tl.to([title, description, bar, separator, logo], {
        opacity: 0,
        duration: EXIT_DURATION,
        ease: "power2.in"
      });
  ...
  resetTargets() {
    if (!this.dom) return;
    gsapWithCSS.set([this.dom.logo, this.dom.separator, this.dom.title, this.dom.description], { opacity: 0 });
    gsapWithCSS.set(this.dom.bar, { clipPath: "inset(0 100% 0 0)" });
  }
}
...
if (!customElements.get(TAG_NAME)) {
  customElements.define(TAG_NAME, FiindoProduktVideo);
}
export {
  FiindoProduktVideo
};
